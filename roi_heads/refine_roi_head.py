...
class DenseContrastLoss(nn.Module):
    ...
    @staticmethod
    @auto_sample
    def dense_contrast_loss(feats, logits, gt_mask, anchor_inds, pos_inds, neg_inds, tau=0.07, margin=0.5):
        '''
        ...
        sim_anchor_pos = torch.matmul(anchor_feats, pos_feats.T) / tau    # A, P
        sim_anchor_neg = torch.matmul(anchor_feats, neg_feats.T) / tau    # A, N

        # Compute the margin term
        margin_term = torch.nn.functional.relu(sim_anchor_neg + margin)

        # Compute the loss
        loss_anchor_pos = torch.nn.functional.cross_entropy(sim_anchor_pos, torch.arange(len(anchor_inds)).to(sim_anchor_pos.device))
        loss_anchor_neg = torch.nn.functional.cross_entropy(-sim_anchor_neg + margin_term, torch.arange(len(anchor_inds)).to(sim_anchor_neg.device))
        ...
        sim_max = sim_anchor_pos.max(1, keepdim=True)[0].detach()   # A, 1
        sim_anchor_pos = torch.cos(sim_anchor_pos - sim_max)
        sim_anchor_neg = torch.cos(sim_anchor_neg - sim_max)

        exp_anchor_neg = sim_anchor_neg.exp().sum(1, keepdim=True)  # A, 1
        exp_anchor_pos = sim_anchor_pos.exp()                    # A, P

        # Compute the loss
        loss_anchor_pos =  torch.log(exp_anchor_pos / (exp_anchor_pos + exp_anchor_neg))
        loss_anchor_neg =  torch.log((-sim_anchor_neg + margin_term) / (exp_anchor_pos + exp_anchor_neg))
        loss = -(loss_anchor_pos + loss_anchor_neg)'''
        ...
        anchor_pos_sim = F.cosine_similarity(anchor_feats, pos_feats)
        anchor_neg_sim = F.cosine_similarity(anchor_feats, neg_feats)

        loss_triplet = torch.relu(anchor_neg_sim - anchor_pos_sim + margin)
